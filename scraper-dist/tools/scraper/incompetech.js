...
exports.IncompetechMusic = void 0;
...
class IncompetechMusic {
    constructor() {
        this.baseUrl = 'https://incompetech.com';
        // 正确的 API 端点：pieces.json 包含所有音乐曲目
        // collections.json 只包含音乐集合/分类信息
        this.musicDataUrl = 'https://incompetech.com/music/royalty-free/pieces.json';
        // 缓存音乐数据
        this.musicCache = null;
        this.cacheTime = 0;
        this.cacheTTL = 3600000; // 1 小时缓存
    }
    /**
     * 获取音乐数据（带缓存）
     */
    ...
    async search(query, options = {}) {
        ...
        let results = musicData.filter(track => {
            ...
            const tagsMatch = track.tags?.some((t) => t.toLowerCase().includes(queryLower));
     ...
     */
    async browseByGenre(genre, maxResults = 20) {
        const musicData = await this.getMusicData();
        const results = musicData.filter(track => track.genre?.toLowerCase() === genre.toLowerCase());
        return results.slice(0, maxResults).map(track => this.formatTrack(track));
    }
    /**
     * 按情绪浏览
     ...
     * @param maxResults 最大结果数
     */
    async browseByMood(mood, maxResults = 20) {
        const musicData = await this.getMusicData();
        // 使用 feel 字段，支持部分匹配（因为 feel 可能包含多个情绪）
        const results = musicData.filter(track => track.feel?.toLowerCase().includes(mood.toLowerCase()));
        return results.slice(0, maxResults).map(track => this.formatTrack(track));
    }
    /**
     * 获取所有可用流派
    ...
    async getMoods() {
        ...
        musicData.forEach(track => {
            if (track.feel) {
                ...
                const feelParts = track.feel.split(',').map((f) => f.trim());
                feelParts.forEach((f) => {
                    if (f)
                        moods.add(f);
                });
...
exports.IncompetechMusic = IncompetechMusic;
//# sourceMappingURL=incompetech.js.map