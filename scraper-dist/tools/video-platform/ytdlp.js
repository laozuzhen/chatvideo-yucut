...
class YtDlpWrapper {
    constructor(options) {
        ...
        this.defaultOutputDir = './storage/videos';
     ...
     * 解析视频信息
     */
    parseVideoInfo(data, originalUrl) {
    ...
    async download(url, options = {}) {
        const { outputDir = this.defaultOutputDir, format = 'best', audioOnly = false, filename, cookies, proxy } = options;
        // 确保输出目录存在
     ...
     * 查找下载的文件
     */
    findDownloadedFile(dir, title) {
        try {
            const files = fs.readdirSync(dir);
            const sanitizedTitle = title.replace(/[<>:"/\\|?*]/g, '');
            // 查找匹配的文件
            const matchedFile = files.find(f => f.includes(sanitizedTitle) ||
     ...
     * @param outputDir 输出目录
     */
    async downloadThumbnail(url, outputDir) {
     ...
     */
    async search(platform, query, maxResults = 10, sortBy = 'relevance', timeout = 60000) {
     ...
     * @param timeout 超时时间
     */
    async searchBilibili(query, maxResults, timeout) {
        // B站搜索 API
        ...
        const buvid3 = Array.from({ length: 35 }, () => 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'[Math.floor(Math.random() * 62)]).join('') + 'infoc';
        try {
     ...
     * @param timeout 超时时间
     */
    async searchYouTube(query, maxResults, sortBy, timeout) {
        ...
        try {
            ...
            return basicResults.map(data => {
                ...
                if (data.view_count !== undefined)
                    result.viewCount = data.view_count;
                return result;
            });
        }
        catch (error) {
