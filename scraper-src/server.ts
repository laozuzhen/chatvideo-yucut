...
// 爬虫模块
import { MixkitScraper } from './tools/scraper/mixkit';
import { IncompetechMusic } from './tools/scraper/incompetech';
import { PexelsScraper } from './tools/scraper/pexels';
import { YtDlpWrapper } from './tools/video-platform/ytdlp';
  ...
  try {
    ...
    if (type === 'all' || type === 'video') {
      try {
        ...
        errors.push(`video: ${(e as Error).message}`);
      }
    }
    ...
    if (type === 'all' || type === 'music') {
      try {
        ...
        errors.push(`music: ${(e as Error).message}`);
      }
    }
    ...
    if (type === 'all' || type === 'image') {
      try {
        ...
        errors.push(`image: ${(e as Error).message}`);
      }
    }
...
app.post('/api/download', async (req, res) => {
  try {
    ...
    // 调用现有的下载工具
    const result = await handleToolCall('download_media', {
      url,
      type,
      filename
    }, null);
...
app.post('/api/ytdlp', async (req, res) => {
  try {
    ...
    // 转换本地路径为 HTTP URL
    const localUrl = `http://localhost:${PORT}/storage/videos/${path.basename(result.localPath || '')}`;
